...
public class FrustumCullingFilter {
    ...
    public void filterVehicle(Map<? extends Mesh, List<Vehicle>> mapMesh) {
        for (Map.Entry<? extends Mesh, List<Vehicle>> entry : mapMesh.entrySet()) {
            List<Vehicle> gameItems = entry.getValue();
            filterVehicle(gameItems, entry.getKey().getBoundingRadius());
        }
    }

    public void filterVehicle(List<Vehicle> gameItems, float meshBoundingRadius) {
        float boundingRadius;
        Vector3f pos;
        for (Vehicle gameItem : gameItems) {
            /*if (!gameItem.isDisableFrustumCulling()) {
                boundingRadius = gameItem.getScale() * meshBoundingRadius;
                pos = gameItem.getPosition();
                gameItem.setInsideFrustum(insideFrustum(pos.x, pos.y, pos.z, boundingRadius));
